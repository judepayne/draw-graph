...
(defn ->xml [parsed]
  #?(:clj (xml/emit-str parsed)
     :cljs (let [raw (xml/emit-str parsed)
                 header-fixed (clojure.string/replace-first raw wrong-svg-header right-svg-header)
                 xlink-fixed (clojure.string/replace header-fixed wrong-xlink "xlink")
                 relics-fixed (clojure.string/replace xlink-fixed relic-ns "")]
             relics-fixed)))
