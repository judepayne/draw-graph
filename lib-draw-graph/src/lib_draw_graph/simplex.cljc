...
(defn is-conform-constraint?
  ...
  (let [{:keys [coefs limit]} a-constraint
        solution-val (reduce + (map * coefs solution))]
    (constraint-direction solution-val limit)))


(defn is-realisable-solution?
  ;; Tests that solution is realisable,
  ;; i.e. every constraint conforms
...
(defn valid-objective?
  ;; all coefficients must be postive
  [objective]
  (every? (partial < 0) objective))


(defn objective-val
  ;; compute objective val for solution
  [solution objective]
  (reduce +
          (map *
               solution
...
(defn complete-p-solution-w-zeroes
  ...
  ;; the toal number of variable nvars
  ;; yields a vector with partial solution
  ;; and remaining var at 0; this will be used
  ;; to test if the current branch is realisable.
  ;; If not, we go up in the tree.
...
(defn branch-n-bound
  [constraints
   ...
   bounding-fn
   incumbent-default]
  {:pre [every? valid-constraint-coefs? constraints
